...
Windows 桌面截图工具 MVP，对标微信截图核心体验。当前进度：T01-T11 已完成（11/18），**阶段 3（UI 层）收尾**（5/5）。完整路线图见 `docs/architecture.md` §6 §7。
...
.venv/Scripts/python.exe scripts/verify_selection_drag.py    # 人工拖拽选区
.venv/Scripts/python.exe scripts/verify_overlay_confirm.py   # T09 QTest 自动化
.venv/Scripts/python.exe scripts/verify_overlay_hide.py      # T10 QTest 自动化
.venv/Scripts/python.exe scripts/verify_integration.py       # T11 端到端集成
...
- `docs/architecture.md`（v1.5）—— 唯一架构真相；每次 Task 完成都要同步（§3.x 模块描述、§5 状态、§6 剩余、§7 阶段、关键决策、版本号）
...
- `src/overlay.py` —— T07-T10 阶段 `OverlayWindow(QWidget)` 单类；保持纯 UI 职责，不串联业务模块（T11 决策）
- `scripts/verify_integration.py` —— T11 端到端集成入口（业务串联发生在脚本内，不在 overlay.py）
